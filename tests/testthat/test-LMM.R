...
test_that("Expected results are obtained for simulated data", {
  ...
  # Snapshot value verified against plotted pedigrees for families 4 and 5
  # in population 1
  ...
  sigma_mat_cond_pr <- sigma_mat[non_pr_ids, non_pr_ids] -
    sigma_mat_non_pr_pr %*%
    Matrix::solve(sigma_mat_pr_pr, Matrix::t(sigma_mat_non_pr_pr))
  sigma_mat_cond_pr_chol <- Matrix::t(Matrix::chol(sigma_mat_cond_pr))
  ...
  sim_rep <- function(r) {
    ...
    rep_data[
      ...
      c("eta", "y") := {
        ...
        y <- eta + as.numeric(sigma_mat_cond_pr_chol %*% rnorm(.N))
        list(eta, y)
      }
    ]
  ...
  p_val_edf_cdf_check <- function(colname, test, data) {
  ...
  resid_dx_edfs_cdf_check <- function(colname, test, data) {
    ...
    edfs_cdf_plot <- ggplot() +
      geom_line(
        aes(x = t, y = edf - cdf, group = rep, color = "Individual\nReplicate"),
        data = plot_data,
        alpha = 0.10
      ) +
      ...
      ylab(paste(
        "EDF - Standard", ifelse(identical(cdf, pnorm), "Normal", "Uniform"),
        "CDF"
      ))
    expect_doppelganger(
  ...
  for (test in seq(1, n_tests)) {
    ...
    resid_dx_edfs_cdf_check("p_c_star_hat", test, p_c_star_hat)
  }
